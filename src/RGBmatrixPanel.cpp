...
// be specified as any pin within a specific PORT register stated below.
  ...
  #define pinSetFastB(_pin)	GPIOB->regs->BSRR = (1U << _pin);
  #define pinResetFastB(_pin) GPIOB->regs->BSRR = (1U << _pin) << 16;
  #define pinSetFastA(_pin)	GPIOA->regs->BSRR = (1U << _pin);
  #define pinResetFastA(_pin) GPIOA->regs->BSRR = (1U << _pin) << 16;
												 
  #define R1	15		// bit 2 = RED 1
  #define G1	14		// bit 3 = GREEN 1
  #define B1	13		// bit 4 = BLUE 1
  #define R2	12		// bit 5 = RED 2
  #define G2	11		// bit 6 = GREEN 2
  #define B2	10		// bit 7 = BLUE 2
  //FASTER
...
void RGBmatrixPanel::begin(void) {
  ...
  gpio_set_mode(GPIOA,_sclk,GPIO_OUTPUT_PP); pinResetFastA(_sclk);	//Low
  gpio_set_mode(GPIOA,_latch,GPIO_OUTPUT_PP); pinResetFastA(_latch);	//Low
  gpio_set_mode(GPIOA,_oe,GPIO_OUTPUT_PP); pinSetFastA(_oe);		//High  (disable output)
  gpio_set_mode(GPIOA,_a,GPIO_OUTPUT_PP); pinResetFastA(_a);		//Low
  gpio_set_mode(GPIOA,_b,GPIO_OUTPUT_PP); pinResetFastA(_b);		//Low
  gpio_set_mode(GPIOA,_c,GPIO_OUTPUT_PP); pinResetFastA(_c);		//Low
  if(nRows > 8) {
    gpio_set_mode(GPIOA,_d,GPIO_OUTPUT_PP); pinResetFastA(_d);		//Low
  }
  gpio_set_mode(GPIOB,15,GPIO_OUTPUT_PP); pinResetFastB(R1);			//Low
  gpio_set_mode(GPIOB,14,GPIO_OUTPUT_PP); pinResetFastB(G1);			//Low
  gpio_set_mode(GPIOB,13,GPIO_OUTPUT_PP); pinResetFastB(B1);			//Low
  gpio_set_mode(GPIOB,12,GPIO_OUTPUT_PP); pinResetFastB(R2);			//Low
  gpio_set_mode(GPIOB,11,GPIO_OUTPUT_PP); pinResetFastB(G2);			//Low
  gpio_set_mode(GPIOB,10,GPIO_OUTPUT_PP); pinResetFastB(B2);			//Low

  timer.pause();
  timer.setPeriod(200);
  timer.setChannel1Mode(TIMER_OUTPUT_COMPARE);
  timer.setCompare(TIMER_CH3, 1);  // Interrupt 1 count after each update
  timer.attachCompare1Interrupt(refreshISR);
  timer.refresh();
  timer.resume();
}
void RGBmatrixPanel::timerpause(void){
	timer.pause();
	timer.refresh();
}
void RGBmatrixPanel::timerresume(void){
	timer.resume();
}
...
void RGBmatrixPanel::updateDisplay(void) {
  ...
  pinSetFastA(_oe);			// Disable LED output during row/plane switchover
  pinSetFastA(_latch);		// Latch data loaded during *prior* interrupt
  pinResetFastA(_sclk);		// Start the clock LOW
  ...
  if(++plane >= nPlanes) {      // Advance plane counter.  Maxed out?
    ...
    if(row & 0x1) {pinSetFastA(_a);}else{ pinResetFastA(_a);}
    if(row & 0x2) {pinSetFastA(_b);}else{ pinResetFastA(_b);}
    if(row & 0x4) {pinSetFastA(_c);}else{ pinResetFastA(_c);}
    if(nRows > 8) {
    if(row & 0x8) {pinSetFastA(_d);}else{pinResetFastA(_d);}
    }
  }
  ...
  pinResetFastA(_oe);		// Re-enable output
  pinResetFastA(_latch);		// Latch down

  if(plane > 0) {

    // Planes 1-3 must be unpacked and bit-banged
    for (i=0; i < WIDTH; i++) {
    GPIOB->regs->BRR = 0b1111110000000000; //set low
		if(ptr[i] & 0x04) { GPIOB->regs->BSRR = (1U << R1); }//else{ pinResetFastB(R1);}		//R1
		if(ptr[i] & 0x08) { GPIOB->regs->BSRR = (1U << G1); }//else{ pinResetFastB(G1);}	//G1
		if(ptr[i] & 0x10) { GPIOB->regs->BSRR = (1U << B1); }//else{ pinResetFastB(B1);}		//B1
		if(ptr[i] & 0x20) { GPIOB->regs->BSRR = (1U << R2); }//else{ pinResetFastB(R2);}		//R2
		if(ptr[i] & 0x40) { GPIOB->regs->BSRR = (1U << G2); }//else{ pinResetFastB(G2);}		//G2
		if(ptr[i] & 0x80) { GPIOB->regs->BSRR = (1U << B2); }//else{ pinResetFastB(B2);}		//B2
		pinSetFastA(_sclk);		//hi
		pinResetFastA(_sclk);	//lo
	...
	for(i=0; i < WIDTH; i++) {
        ...
        GPIOB->regs->BRR = 0b1111110000000000; //set low
		if(bits & 0x04) { GPIOB->regs->BSRR = (1U << R1); }//else{ pinResetFastB(R1);}		//R1
		if(bits & 0x08) { GPIOB->regs->BSRR = (1U << G1); }//else{ pinResetFastB(G1);}	//G1
		if(bits & 0x10) { GPIOB->regs->BSRR = (1U << B1); }//else{ pinResetFastB(B1);}		//B1
		if(bits & 0x20) { GPIOB->regs->BSRR = (1U << R2); }//else{ pinResetFastB(R2);}		//R2
		if(bits & 0x40) { GPIOB->regs->BSRR = (1U << G2);}//else{ pinResetFastB(G2);}		//G2
		if(bits & 0x80) { GPIOB->regs->BSRR = (1U << B2); }//else{ pinResetFastB(B2);}		//B2
		pinSetFastA(_sclk);		//hi
		pinResetFastA(_sclk);	//lo
